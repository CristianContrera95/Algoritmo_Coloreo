...
u32 Greedy(WinterIsHere W){
  ...
  for(u32 j = 0; j < lengVertex - 1; j++){
    ...
    gradeVert = vertex->grade; // Grado del vertice actual
    maxColor = 0;
    colorVertice = 0;
    ...
    if(!maxColor) // Si no tenemos maxColor es porque ningun color fue usado
      ...
      for(u32 i = 1; i <= maxColor + 1; i++){
        // Recorremos colores buscando un 0 que indica un color disponible, en el 
        // camino vamos reseteando el arreglo
        if(colores[i]){
          colores[i] = 0;
          continue;
        }
